...
| Export | `export.py`, `data_requests.py`, `explain.py`, `seed_paths.py` | Required tables, observation gaps, rule traces and estimated flow paths |
| Benchmark | `bench.py` | Reproducible scale measurements and chart, separate from normal runs |
...
- `agent/store.py` provides deterministic, read-only queries over exported case evidence.
...
- `agent/cards.py` formats deterministic client facts from exported evidence; optional
  prose generation returns a summary without writing to the read-only output snapshot.
...
- `app/app.py` owns Streamlit page navigation. `ui.py` provides shared controls and
  cached exports; `pages/` owns individual screens and `dossier.py` renders client
  evidence. `charts.py` and `graphview.py` produce charts and selectable SVG graphs.
  `theme.py`, `style.css` and licensed fonts under `static/` define presentation.

The viewer and assistant never assign roles or priorities. Optional assistant
evaluation is separate from pipeline results. The Method view
...
Additional exports include features and transfer parquet tables, resilience, data
requests, the truncation model, rule traces, estimated seed paths and pipeline
metadata. They form the reviewed case snapshot under `out/`. The viewer and
assistant read those exports without reopening the original input data. Benchmark
artifacts are generated separately by `make bench`; assistant evaluations remain local.
...
Compare artifacts before and after analytical refactors using the same inputs and
configuration; exclude measured runtime and environment metadata from byte comparisons.
