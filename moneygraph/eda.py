"""Prints the data findings quoted in docs/design.md."""
import argparse
...
def main():
    ...
    print("depth 1-3 sinks with inflow:", int(((d < 4) & (out_deg[d.index] == 0) & (in_deg[d.index] > 0)).sum()))

    fi, fo = T.groupby("dst").date.min(), T.groupby("src").date.min()
    both = fi.index.intersection(fo.index)
    print(f"intermediaries: {len(both)}, first out before first in: {int((fo[both] < fi[both]).sum())}")

    dd = E[["src", "dst"]].assign(depth_src=E.src.map(d), depth_dst=E.dst.map(d))
    print("back/lateral edges:", int((dd.depth_dst <= dd.depth_src).sum()),
          " into seeds:", int(E.dst.isin(ctx.seeds).sum()))
    print("max in/out degree:", int(in_deg.max()), int(out_deg.max()), " in_deg>=5:", int((in_deg >= 5).sum()))
    print("weak components:", [len(c) for c in sorted(nx.weakly_connected_components(G), key=len, reverse=True)][:5])
