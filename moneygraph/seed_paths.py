...
def build(ctx):
    ...
    for _ in range(rounds):
        ...
        for i, gid in enumerate(nodes):
            ...
            for j, share in outgoing[i]:
                next_paths[j].extend(
                    (amount * share, (*path, nodes[j]))
                    for amount, path in emitted
                    if amount * share > 0
                )
        ...
        incoming = [
            nlargest(limit, paths, key=lambda p: (p[0], tuple(map(str, p[1]))))
            for paths in next_paths
        ]
    ...
    for i, gid in enumerate(nodes):
        ...
        result[str(gid)] = {
            "seed_flow_in": float(totals[i]),
            "shown_kzt": float(shown),
            "omitted_kzt": float(max(0, totals[i] - shown)),
            "coverage": float(min(1, shown / totals[i])) if totals[i] > 0 else 0.0,
            "paths": [
                {
                    "gids": list(map(str, path)),
                    "kzt": float(amount),
                    "hops": len(path) - 1,
                    "contains_cycle": len(path) != len(set(path)),
                }
                for amount, path in incoming[i]
            ],
        }
    return {
        "schema_version": 1,
        ...
        "limitations": [
            "Attribution is modeled, not traced transactions or proof of ownership.",
