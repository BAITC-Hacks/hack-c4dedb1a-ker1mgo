...
def condition(field, label, value, operator, threshold, source, *, applicable=True, scored=True):
    ...
    return {"field": field, "label": label, "value": float(value) if available else None,
            "operator": operator, "threshold": threshold, "threshold_source": source,
            "passed": bool(passed), "applicable": bool(applicable), "available": bool(available),
            "deficit": float(deficit) if deficit is not None else None,
            "distance": float(distance), "margin": margin, "scored": scored}


def group(operator, *children, scored=True):
    matched = all(c["passed"] for c in children) if operator == "all" else any(c["passed"] for c in children)
    valid = [c["margin"] for c in children if c["passed"] and c["scored"] and c["margin"] is not None]
    margin = (min(valid) if operator == "all" else max(valid)) if valid and matched else None
    distance = sum(c["distance"] for c in children) if operator == "all" else min(c["distance"] for c in children)
    return {"operator": operator, "children": list(children), "passed": matched,
            "margin": margin, "scored": scored, "distance": distance}
...
def rule_definition(name, r, c, bt_thr):
    ...
    if name == "coordinator":
        ...
        return group("all", group("any",
            pred("pays_seed", "Seeds paid back", ">=", k["min_pays_seed"], "roles.coordinator.min_pays_seed"),
            pred("cycle_with_seeds", "Other seeds on short cycles", ">=", k["min_cycle_seeds"], "roles.coordinator.min_cycle_seeds")),
            group("any",
            pred("n_seed_sources", "Reachable seed sources", ">=", k["min_seed_sources"], "roles.coordinator.min_seed_sources"),
            pred("betweenness", "Betweenness", ">=", bt_thr, "roles.coordinator.betweenness_pct")))
    if name == "distributor":
        k = c[name]
        return group("all",
            pred("out_deg", "Recipients", ">=", k["min_out_deg"], "roles.distributor.min_out_deg"),
            pred("out_deg", "Recipients versus payers", ">=", k["out_in_ratio"] * max(r.in_deg, 1), "roles.distributor.out_in_ratio × max(in_deg, 1)"))
    if name == "consolidator":
        k = c[name]
        return group("all",
            pred("in_deg", "Payers", ">=", k["min_in_deg"], "roles.consolidator.min_in_deg"),
            group("any",
            pred("pass_through", "Outflow / observed inflow", "<=", k["max_pass_through"], "roles.consolidator.max_pass_through", applicable=not r.is_seed),
            pred("out_deg", "Recipients versus payers", "<=", r.in_deg * k["out_in_ratio"], "roles.consolidator.out_in_ratio × in_deg")))
    if name == "transit":
        k = c[name]
        return group("all", nonseed(),
            ...
            group("any",
            pred("pass_through", "Outflow / observed inflow band", "between", [k["pass_through_low"], k["pass_through_high"]], "roles.transit.pass_through_low/high", applicable=not r.is_seed),
            group("all",
            pred("fast_pass_share", "Inflow forwarded within the configured time window", ">=", k["min_fast_pass_share"], "roles.transit.min_fast_pass_share", applicable=not r.is_seed),
            pred("pass_through", "Fast-forwarding outflow cap", "<=", k["max_pass_through_fast"], "roles.transit.max_pass_through_fast", applicable=not r.is_seed, scored=False))))
    if name == "terminal":
        return group("all", nonseed(),
            pred("out_deg", "No observed recipients", "==", 0, "graph.no_outgoing", scored=False),
            pred("in_deg", "At least one payer", ">=", 1, "graph.has_incoming", scored=False),
            group("any",
            pred("depth", "Outgoing transfers were crawled", "<=", 3, "crawl.observed_depth", scored=False),
            pred("p_has_out", "Estimated forwarding probability", "<", c[name]["max_p_has_out"], "roles.terminal.max_p_has_out", applicable=r.depth > 3 and not r.is_seed, scored=False)))
...
def explain(r, c, bt_thr):
    ...
    for name in RULES:
        tree = rule_definition(name, r, c, bt_thr)
        rules.append({"role": name, "matched": tree["passed"], "distance": tree["distance"],
                      "conditions": flat_conditions(tree), "logic": tree})
    ...
    return {"matched_rule": next((item["role"] for item in rules if item["matched"]), None),
            "nearest_rule": nearest, "rules": rules}
...
def fallback(r, c):
    ...
    likely = condition("p_has_out", "Estimated forwarding probability", r.p_has_out,
                       ">", c["truncated_likely_forwarding_p"], "roles.truncated_likely_forwarding_p")
    candidates = [("seed_no_outgoing", group("all", seed, no_out)),
...
def assign(r, c, bt_thr, trace=None):
    ...
    matched = {item["role"]: m for item in evaluated
...
def compute(ctx) -> pd.DataFrame:
    ...
    for values, top in zip(f.itertuples(index=False, name=None), bt_top):
        ...
        trace.update(role=role, role_detail=detail, secondary_roles=secondary.split(";") if secondary else [])
        if role == "peripheral":
            _, tree = fallback(r, c)
            trace["fallback"] = {"role": detail, "matched": True, "distance": 0.0,
                                 "logic": tree, "conditions": flat_conditions(tree)}
