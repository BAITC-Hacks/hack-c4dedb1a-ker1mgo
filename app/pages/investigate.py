...
page_header(
    "Исследование связей",
    "Найдите клиента и выберите узел на графе, чтобы проверить его роль, переводы и источники средств.",
)
...
def investigation():
    ...
    query = search.text_input(
        "ID клиента", placeholder="Полный ID или последние цифры", key="case_search"
    )
    radius = (
        radius_col.segmented_control(
            "Глубина связей",
            [1, 2],
            default=1,
            format_func=lambda r: "1 шаг" if r == 1 else "2 шага",
            key="graph_radius",
        )
        or 1
    )
    color_by = (
        colour_col.segmented_control(
            "Цвет узлов",
            ["role", "cluster"],
            default="role",
            format_func=lambda v: "Роли" if v == "role" else "Кластеры",
            key="graph_color",
        )
        or "role"
    )
    ...
    if query:
        ...
        if not hits:
            ...
            st.caption(
                f"Найдено совпадений: {len(hits)}"
                + (" или больше. Уточните ID." if len(hits) == 100 else ". Выберите клиента.")
            )
    ...
    with graph:
        st.html(
            f'<div class="panel-title">Карта переводов<span class="panel-meta">{number(len(G))} узлов / {number(G.number_of_edges())} связей</span></div>'
        )
        result = render(G, store.f, focus=gid, color_by=color_by, height=380, key="case_graph")
        ...
        if len(G) >= 300:
            st.caption(
                "Показаны 300 ближайших узлов; при одинаковой глубине выбраны крупнейшие потоки."
            )
    with card, st.container(key="dossier_summary"):
