...
from app.ui import badge, get_store, open_dossier, page_header

store = get_store()
f = store.f
page_header(
    "Картина движения денег",
    "От известных исходных точек — к клиентам, связям и запросам, которые стоит проверить следующими.",
)
...
with left:
    ...
    badge(node["role"])
    st.html(
        f'<div class="case-note"><div class="dossier-id">{node["gid"]}</div>{html.escape(summary(node))}</div>'
    )
    ...
    labels = {
        "biggest distributor": "Крупный распределитель",
        "depth 4, inferred terminal": "Получатель на границе обхода",
        "top coordinator": "Признаки координации",
    }
...
for column, title, text in zip(
    ...
    [
        f"{number(depth4)} клиента без исследованных исходящих",
        "Наблюдение отдельно от предположения",
    ],
    [
...
findings = [
    (
        "Следуем за средствами",
        "Приоритет учитывает расчётный поток от seeds. В досье видно, из чего складывается балл и какие пути ведут к клиенту.",
    ),
    (
        "Проверяем полезность приоритета",
        "Сравниваем удаление узлов по приоритету, числу связей и случайному выбору. Результаты доступны в разделе «Метод и масштаб».",
    ),
    (
        "Формируем следующий запрос",
        "Неизвестные исходящие переводы, неполные поступления и отдельные компоненты превращаются в конкретные запросы данных.",
    ),
    (
        "Задаём вопросы по графу",
        "Опциональный ассистент обращается к готовым данным и проверяет ID в ответах. Для основного исследования подключение не требуется.",
    ),
]
