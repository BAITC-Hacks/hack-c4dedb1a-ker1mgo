...
store = get_store()
page_header("Who to review first, and why.", "A ranked review queue based on seed money, role evidence, connected sources and removal impact.")
...
st.caption("Select a row to open its dossier. Scores order analyst review and are not probabilities of wrongdoing.")
node_table(rows, "priority_queue", ["gid", "role", "priority_score", "why", "cluster_id"])
st.download_button("Download this review queue", rows.to_csv(index=False), "review_queue.csv", "text/csv")
