...
store = get_store()
page_header("Ask a question. Inspect the references.", "The optional assistant queries the case outputs and checks every cited client ID against the graph.")
try:
    ...
    available = False
if not available:
    st.info("The assistant is not connected. To enable it, set `OPENAI_API_KEY` in `.env` and restart the app. Investigations, dossiers and all other pages work locally without a key.")
    st.markdown("**Questions you can ask when connected**")
    st.write("Who receives money from these clients? Which distributors should I review first? What evidence supports this cluster hypothesis?")
...
if not chat:
    examples = [store.demo_question(), "Who are the top distributors?", "Кого проверить первым и почему?"]
...
if question:
    ...
    with st.spinner("Checking the case evidence…"):
        try:
            ...
            chat.append({"role": "assistant", "text": "The assistant could not complete this request. Check the model connection and try again, or use Investigate to inspect the evidence locally."})
    st.rerun()
