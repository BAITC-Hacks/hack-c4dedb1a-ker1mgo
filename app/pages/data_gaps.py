...
store = get_store()
page_header("Запросы недостающих данных", "Выберите пробел в наблюдениях, проверьте досье и сформируйте запрос следующей выгрузки.")
...
reason = st.selectbox("Тип запроса", ["all"] + sorted(requests.reason.unique()), format_func=lambda v: "Все запросы" if v == "all" else REQUESTS.get(v, (v, ""))[0])
...
st.download_button("Скачать запросы · CSV", rows.to_csv(index=False), "data_requests.csv", "text/csv")
st.caption("Переводы менее 5 000 KZT и операции за пределами банка не входят в выборку. Их нельзя восстановить по видимым связям.")
