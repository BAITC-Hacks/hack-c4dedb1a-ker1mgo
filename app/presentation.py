...
DETAILS = {
    "terminal_observed": "Исходящие переводы проверены",
    "terminal_inferred": "Вывод модели · требует проверки",
    "truncated_unknown": "Исходящие переводы неизвестны",
    "truncated_likely_forwarding": "Возможное продолжение потока",
    "seed_no_outgoing": "Исходная точка без исходящих переводов",
    "no_edges": "Нет видимых переводов",
    "weak_signal": "Недостаточно структурных признаков",
}
CONDITIONS = {
    "Seeds paid back": "Переводы исходным точкам",
    "Other seeds on short cycles": "Исходные точки в коротких циклах",
    "Reachable seed sources": "Доступные исходные точки",
    "Betweenness": "Посредничество в сети",
    "Recipients": "Получатели",
    "Recipients versus payers": "Получатели относительно плательщиков",
    "Payers": "Плательщики",
    "Outflow / observed inflow": "Доля отправленных средств",
    "Known seed": "Исходная точка (seed)",
    "At least one payer": "Есть входящие связи",
    "At least one recipient": "Есть исходящие связи",
    "Outflow / observed inflow band": "Доля отправленных средств",
    ...
    "No observed recipients": "Число видимых получателей",
    "Outgoing transfers were crawled": "Глубина проверенного обхода",
...
REQUESTS = {
    "truncated_likely_forwarding": (
        "Продолжить обход",
        "Запросить исходящие переводы на следующем, пятом шаге обхода.",
    ),
    "seed_inflow_undercounted": (
        "Уточнить источники средств",
        "Запросить полную историю входящих переводов исходной точки.",
    ),
    "seed_no_outgoing": (
        "Проверить исходную точку",
        "Запросить историю переводов и проверить другие каналы: исходящих связей в выборке нет.",
    ),
    "small_component": (
        "Проверить отдельную группу",
        "Расширить историю переводов небольшой компоненты и проверить связи за пределами выборки.",
    ),
}
FLAGS = {
    "burst": "Всплеск операций за один день",
    "repeat_amount": "Повторяющиеся суммы",
    "round_amounts": "Округлённые суммы",
    "near_threshold": "Переводы около порога выгрузки",
}
COLUMNS = {
    "gid": "ID клиента",
    "role": "Гипотеза роли",
    "priority_score": "Приоритет",
    "why": "Основание для проверки",
    "cluster_id": "Кластер",
    "evidence": "Наблюдаемые признаки",
    "reason": "Причина запроса",
    "suggested_request": "Что запросить",
    "sum_kzt": "Сумма, KZT",
    "n_tx": "Переводы",
    "direction": "Направление",
    "is_seed": "Исходная точка",
    "n_nodes": "Клиенты",
    "n_seed": "Исходные точки",
    "sum_kzt_internal": "Оборот внутри, KZT",
    "hypothesis": "Гипотеза",
    "seed_flow_in": "Поток от seeds, KZT",
}
...
def logic_text(text):
    ...
    return (
        text.replace(" AND ", " И ")
        .replace(" OR ", " ИЛИ ")
        .replace("No primary role rule matched", "Ни одно основное правило не выполнено")
    )
...
def summary(node):
    ...
    if role in {"consolidator", "transit"}:
        ...
        return text + (
            " Картина согласуется с консолидацией средств."
            if role == "consolidator"
            else " Картина согласуется с транзитом средств."
        )
...
def cluster_hypothesis(text):
    translations = [
        (r"possible collection cell", "Возможная группа сбора средств"),
