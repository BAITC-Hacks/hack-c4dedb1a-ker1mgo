...
from app.presentation import number  # noqa: E402
...
pages = [
    st.Page(
        "pages/briefing.py", title="Обзор дела", default=True, icon=":material/space_dashboard:"
    ),
...
with st.sidebar:
    ...
    for item in pages:
        st.page_link(item)
    st.html(
        f'<div class="sidebar-scope"><strong>{number(len(store.f))}</strong> клиентов в выборке<br>Исходные точки: <strong>{int(store.f.is_seed.sum())}</strong></div>'
    )
    with st.expander("Как читать граф"):
        legend()
        st.caption(
            "Стрелка указывает направление перевода. Толщина линии зависит от суммы. Тёмная обводка — исходная точка, пунктир — граница обхода на глубине 4."
        )
    st.html(
        '<div class="sidebar-footnote">Роли и приоритеты — гипотезы для проверки, а не вывод о виновности.</div>'
    )
page.run()
