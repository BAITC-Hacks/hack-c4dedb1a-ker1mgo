...
@pytest.fixture
def store():
    edges = [(p, C, 1000.0 * (i + 1), 1) for i, p in enumerate(P)] + [
        (C, S, 9000.0, 3),
        (SEED, C, 500.0, 1),
    ]
    ...
    tx = pd.DataFrame(
        {
            "src": [11, 20, 20],
            "dst": [20, 30, 30],
            "date": ["2026-07-01", "2026-07-02", "2026-07-02"],
            "sum_kzt": [1000.0, 4000.0, 5000.0],
        }
    )
...
def test_load_uses_exported_evidence(store, tmp_path):
    store.f.to_parquet(tmp_path / "features.parquet", index=False)
    store.edges.to_parquet(tmp_path / "edges.parquet", index=False)
    store.tx.to_parquet(tmp_path / "transactions.parquet", index=False)
    traces = {"nodes": {str(C): {"role": "consolidator"}}}
    paths = {"nodes": {str(C): {"paths": [[SEED, C]]}}}
    model = {"enabled": False}
    for name, content in (
        ("rule_traces", traces),
        ("seed_paths", paths),
        ("truncation_model", model),
    ):
        (tmp_path / f"{name}.json").write_text(json.dumps(content), encoding="utf-8")

    loaded = GraphStore.load(tmp_path)

    assert loaded.get_node(C) == store.get_node(C)
    assert loaded.rule_trace(C) == traces["nodes"][str(C)]
    assert loaded.money_paths(C) == paths["nodes"][str(C)]
    assert loaded.truncation_model == model
    assert loaded.rule_trace(ISO) == loaded.money_paths(ISO) == {}
...
def test_guardrail_retries_then_passes(store):
    ...
    tool_call = AIMessage(
        "",
        tool_calls=[
            {"name": "common_collectors", "args": {"gids": [str(g) for g in P]}, "id": "1"}
        ],
    )
    r = _run(
        store,
        [tool_call, AIMessage("collector 100000000000000555"), AIMessage(f"collector {C:018d}")],
    )
