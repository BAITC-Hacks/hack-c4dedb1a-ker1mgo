...
from agent.tools import cited_gids
...
def make_questions(store, seed=7):
    ...
    for lang, cid in (("ru", "collector_ru"), ("en", "collector_en")):
        ...
        payers = store.neighbors(c, "in").gid.head(5).map(s).tolist()
        text = (f"Кто собирает деньги с этих пятерых: {', '.join(payers)}?" if lang == "ru"
                else f"Who collects money from these clients: {', '.join(payers)}?")
        qs.append((cid, text, [s(c)]))

    # two payers -> recipient they share
    shared = [(v, sorted(store.G.predecessors(v))[:2]) for v in f.index if store.G.in_degree(v) >= 2]
    _, (a, b) = shared[rng.integers(len(shared))]
    both = set(store.G.successors(a)) & set(store.G.successors(b))
    qs.append(("shared_recipient", f"Which clients receive money from both {a} and {b}?", [s(x) for x in both]))

    x = _pick(rng, f.index[f.in_deg.between(2, 4)])
    qs.append(("payers", f"Кто переводит деньги клиенту {x}?", store.neighbors(x, "in").gid.map(s).tolist()))

    x = _pick(rng, f.index[f.out_deg.between(2, 5)])
    qs.append(("recipients", f"To whom does {x} send money?", store.neighbors(x, "out").gid.map(s).tolist()))

    x = _pick(rng, f.index[f.out_deg >= 3])
    top = store.neighbors(x, "out").gid.iloc[0]
    qs.append(("biggest_recipient", f"Кто получил от {x} больше всего денег?", [s(top)]))

    qs.append(("top5", "Which 5 clients should an analyst look at first?", store.top_nodes(5).gid.map(s).tolist()))
    qs.append(("top3_nonseed", "Назови трёх самых приоритетных клиентов, которые не являются seed.",
               store.top_nodes(3, include_seeds=False).gid.map(s).tolist()))

    # a two-hop chain u -> m -> w
    edges = [(u, v) for u in sorted(store.G) for v in sorted(store.G.successors(u)) if store.G.out_degree(v)]
    ...
    qs.append(("path", f"Через кого проходят деньги от {u} к {w}?", [s(g) for g in mids]))
    ...
    if not cl.empty:
        ...
        qs.append(("cluster_top", f"Which cluster is {member} in, and who are its top clients?",
...
def run(model=None, out=OUT):
    ...
    for qid, q, expected in make_questions(store):
        ...
        rows.append({
            "id": qid, "question": q, "expected": ";".join(expected), "cited": ";".join(r["gids"]),
            "recall": round(recall, 3), "precision": round(precision, 3), "n_unknown": len(unknown), "unknown": ";".join(unknown),
            "tools": ";".join(r["tools"]), "steps": r["steps"], "seconds": round(time.perf_counter() - t0, 1),
            "error": err, "answer": r["answer"],
        })
        print(f"{qid:18s} recall {recall:.2f}  precision {precision:.2f}  unknown {len(unknown)}  tools {','.join(r['tools'])}")
